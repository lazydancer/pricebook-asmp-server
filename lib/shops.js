...
const createShopsAdapter = (db) => {
  ...
  const createTopShopsStmt = (priceOrder) => db.prepare(`
    ...
    shop_waystones AS (
      ...
      LEFT JOIN waystones w ON w.dimension = rs.dimension AND w.removed_at IS NULL
  ...
  const getItemHistoryStmt = db.prepare(`
    WITH RECURSIVE dates(day_timestamp, is_today) AS (
      -- Start with end of today (23:59:59.999 in milliseconds)
      SELECT
        CAST(strftime('%s', 'now', 'start of day', '+1 day', '-1 second') AS INTEGER) * 1000,
        1
      UNION ALL
      -- Go back one day at a time (86400000 ms = 1 day)
      SELECT
   ...
   * No deletions - only set removed_at timestamp for missing shops
   */
  ...
  return {
    ...
    getItemHistory: (params) => {
      const rows = getItemHistoryStmt.all({ item: params.item });
      return rows.map(row => ({
        date: row.date,
