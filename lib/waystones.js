 ...
 * History preserved via removed_at timestamp (no deletions).
 */
...
const createWaystonesAdapter = (db) => {
  ...
  const selectCurrentAtPositionStmt = db.prepare(`
    ...
    FROM waystones
    WHERE dimension = ? AND pos_x = ? AND pos_y = ? AND pos_z = ? AND removed_at IS NULL
  `);
   ...
   * No deletions - only set removed_at timestamp for missing waystones
   */
  ...
  const syncChunkWaystones = (chunk, scanRow) => {
    ...
    for (const [posKey, waystone] of existingByPosition) {
      if (!seenPositions.has(posKey)) {
        ...
        markNotCurrentStmt.run(scanRow.scannedAt, waystone.id);
      }
    }
