...
const createLatestWaystonesAdapter = (db) => {
  ...
  const pruneChunkWaystones = (chunk, waystones, scanRow) => {
    const existingRows = selectLatestWaystonesForChunkStmt
      .all(encodeDimension(chunk.dimension), chunk.chunkX, chunk.chunkZ)
      .map(row => ({
        ...row,
    ...
    const existingByPos = new Map(
      existingRows.map(row => [
        keyForWaystone(row.dimension, row.pos_x, row.pos_y, row.pos_z),
        row
      ])
    );

    for (const waystone of waystones) {
      const posKey = keyForWaystone(waystone.dimension, waystone.posX, waystone.posY, waystone.posZ);
  ...
  const reconcileScan = (scanRow, waystoneRows) => {
    ...
    if (chunkWaystones.length === 0) {
      return result;
    }

    const waystonesByChunk = new Map();
    const getOrCreateChunk = (dimension, chunkX, chunkZ) => {
      const key = chunkKey(dimension, chunkX, chunkZ);
      if (!waystonesByChunk.has(key)) {
        waystonesByChunk.set(key, {
          dimension,
          chunkX,
          chunkZ,
          waystones: []
      ...
      return waystonesByChunk.get(key);
    };

    getOrCreateChunk(scanRow.dimension, scanRow.chunkX, scanRow.chunkZ);

    for (const waystone of chunkWaystones) {
      const chunk = getOrCreateChunk(waystone.dimension, waystone.chunkX, waystone.chunkZ);
      chunk.waystones.push(waystone);
    }

    for (const chunk of waystonesByChunk.values()) {
      const pruned = pruneChunkWaystones(chunk, chunk.waystones, scanRow);
