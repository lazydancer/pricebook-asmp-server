...
const { encodeDimension, decodeDimension } = require('./enums');
...
const createLatestWaystonesAdapter = (db) => {
  ...
  const selectByPositionStmt = db.prepare(`
    SELECT *
    FROM latest_waystones
  ...
  const nearestWaystoneStmt = db.prepare(`
    SELECT id, name, owner, dimension, pos_x AS posX, pos_y AS posY, pos_z AS posZ,
  ...
  const reconcileScan = (scanRow, waystoneRows) => {
    ...
    // Track per-chunk observations so we only touch relevant rows when pruning.
    const chunkContexts = new Map();
    const contextFor = (dimension, chunkX, chunkZ) => {
      const key = chunkKey(dimension, chunkX, chunkZ);
      if (!chunkContexts.has(key)) {
        chunkContexts.set(key, {
          dimension,
          chunkX,
          chunkZ,
          seenKeys: new Set()
      ...
      return chunkContexts.get(key);
    };

    contextFor(scanRow.dimension, scanRow.chunkX, scanRow.chunkZ);

    const uiWaystones = [];
    let chunkScanPresent = false;

    for (const waystone of observations) {
      if (waystone.source === 'ui') {
        uiWaystones.push(waystone);
        continue;
      }

      if (waystone.source === 'chunk') {
        chunkScanPresent = true;
        const context = contextFor(waystone.dimension, waystone.chunkX, waystone.chunkZ);
        context.seenKeys.add(
          keyForWaystone(waystone.dimension, waystone.posX, waystone.posY, waystone.posZ)
        );
      }
    }
    ...
    // UI-only scans update metadata but do not prune.
    if (!chunkScanPresent && uiWaystones.length > 0) {
      return result;
    }

    const pruneMissingWaystones = () => {
      const pruned = [];
      for (const context of chunkContexts.values()) {
        const existingRows = selectLatestWaystonesForChunkStmt
          .all(encodeDimension(context.dimension), context.chunkX, context.chunkZ)
          .map((row) => ({
            ...row,
        ...
        for (const row of existingRows) {
          const waystoneKey = keyForWaystone(row.dimension, row.pos_x, row.pos_y, row.pos_z);
          remaining.set(waystoneKey, row);
        }

        for (const observedKey of context.seenKeys) {
          const existing = remaining.get(observedKey);
          if (!existing) {
            continue;
          }
          ...
          remaining.delete(observedKey);
        }

        for (const leftover of remaining.values()) {
          pruned.push({
            id: leftover.id,
            dimension: leftover.dimension,
            posX: leftover.pos_x,
            posY: leftover.pos_y,
            posZ: leftover.pos_z
          });
    ...
    result.prunedWaystones = pruneMissingWaystones();
    return result;
  };

  const upsertObservation = (observation) => {
    const existing = selectByPositionStmt.get(
      encodeDimension(observation.dimension),
      observation.posX,
      observation.posY,
      observation.posZ
    );
  ...
  return {
    ...
    nearestTo: (params) => {
      const row = nearestWaystoneStmt.get({
        ...params,
        dimension: encodeDimension(params.dimension)
      });
      if (!row) {
        return null;
      }
