...
const ensureTableRenamed = (db) => {
  const legacyTable = db.prepare(`
    SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'waystone_latest'
  `).get();

  if (legacyTable) {
    db.exec('ALTER TABLE waystone_latest RENAME TO latest_waystones');
    db.exec(`
      DROP INDEX IF EXISTS idx_waystone_latest_chunk;
      DROP INDEX IF EXISTS uq_waystone_latest_position;
    `);
  }
};
...
const ensureWaystoneLatestSchema = (db) => {
...
const createLatestWaystonesAdapter = (db) => {
  ...
  const nearestWaystoneStmt = db.prepare(`
    SELECT name, pos_x AS posX, pos_y AS posY, pos_z AS posZ,
  ...
  const normalizeField = (incomingValue, existingValue) => {
...
const rebuildLatestWaystones = (db) => {
  ...
  const selectScansStmt = db.prepare(`
    SELECT id, scan_id, dimension, chunk_x, chunk_z, scanned_at
    FROM scans
  ...
  const rebuildTx = db.transaction(() => {
    ...
    for (const scan of scans) {
      const waystoneRows = selectWaystonesForScanStmt.all(scan.scan_id).map((row) => {
